...
#include "marlinui.h"
MarlinUI ui;
 bool MarlinUI::AC_bar_control; 
 char MarlinUI::start_flag;
 extern float Beforeprobe_offset;
...
constexpr uint8_t epps = ENCODER_PULSES_PER_STEP;
  ...
  uint8_t MarlinUI::language = 1; // Initialized by settings.load()
  ...
  bool MarlinUI::get_blink() {
    ...
    if (ELAPSED(ms, next_blink_ms)) {
      blink ^= 0xFF;
      next_blink_ms = ms + 300 - (LCD_UPDATE_INTERVAL) / 2;
    }
    return blink != 0;
  }
...
//      void lcd_move_z();
   ...
   */
    ...
    #endif
  #endif
//*******************************因为要在这里加选择项，所以需要增加一下变量，表示被选中的那个项，然后那个项就顺便颜色变成闪烁型*************
//************************0表示没有选中，点击就会进入主界面*********************
//************************1表示选中喷嘴，点击就修改喷嘴温度*********************
//************************2表示热床，3风扇，4打印速度，5速比，6暂无***************
...
//        break;
        case 1:
          ...
          goto_screen(lcd_setTargetHotend);
          ui.refresh();                         //
        ...
        case 3:
         ...
         case 4:
          ...
          goto_screen(lcd_babystep_zoffset);    //z 轴偏移
          ui.refresh();                         //
          seclect=4;
        ...
        case 5:
          ...
          goto_screen(Lcd_set_feedrate);    
          ui.refresh();                         //
          seclect=5;
        
        break;
        case 6:
//          goto_screen(Lcd_set_flow);
		  if(busy) goto_screen(menu_tune);
		  else goto_screen(menu_main);
...
//                 seclect +=int16_t(encoderPosition);
//                  if(seclect<0)
//                  {
//                     if((!busy)&&(!Paused))
//                     {
...
//              
//             }
       if (int16_t(encoderPosition) >= ENCODER_item_Move)
         {
                seclect++;
               if((!busy)&&(!Paused))
               {
         ...
         else if (int16_t(encoderPosition) <= -(ENCODER_item_Move))
          {
                  if(seclect==1)
                  {
                        if((!busy)&&(!Paused))
...
//         else if(seclect==3)
//         {
//                  old_data = thermalManager.fan_speed[0];
//                  new_data = old_data + (int16_t(encoderPosition));
//                    LIMIT(new_data, 0, 255);

//                    if (old_data != new_data) {
//                      thermalManager.fan_speed[0] = new_data;
//                      encoderPosition = 0;
...
//                  old_data = feedrate_percentage;
//                  new_data = old_data + int16_t(encoderPosition);

//                // Dead zone at 100% feedrate
//                    if (old_data == 100) {
//                      if (int16_t(encoderPosition) > ENCODER_FEEDRATE_DEADZONE)
//                        new_data -= ENCODER_FEEDRATE_DEADZONE;
//                      else if (int16_t(encoderPosition) < -(ENCODER_FEEDRATE_DEADZONE))
//                        new_data += ENCODER_FEEDRATE_DEADZONE;
//                      else
//                        new_data = old_data;
//                    }
//                    else if ((new_data < 100 && new_data > 100) || (new_data > 100 && new_data < 100))
//                      new_data = 100;

//                    LIMIT(new_data, 10, 999);

//                    if (old_data != new_data) {
//                      feedrate_percentage = new_data;
//                      encoderPosition = 0;
//                      #if BOTH(HAS_BUZZER, BEEP_ON_FEEDRATE_CHANGE)
//                        static millis_t next_beep;
//                        #ifndef GOT_MS
...
//      else if(seclect==5)
//      {
//                  old_data = planner.flow_percentage[0];
//                  new_data = old_data + int16_t(encoderPosition);
...
//                    if (old_data != new_data) {
//                      planner.flow_percentage[0] = new_data;
//                      encoderPosition = 0;
//                      #if BOTH(HAS_BUZZER, BEEP_ON_FEEDRATE_CHANGE)
//                        static millis_t next_beep;
//                        #ifndef GOT_MS
...
void check_endstops()
{
       ...
       static millis_t endstop_ms = 0;
       static bool endstops_hit = 0;
       if(printingIsActive() || endstops.enabled) return;
       ...
       static uint8_t cur_endstops_state = x_state<<2 | y_state<<1 | z_state;
       static uint8_t prv_endstops_state = cur_endstops_state;

       cur_endstops_state = x_state<<2 | y_state<<1 | z_state;
       ...
       if(!endstops.enabled){
               char cmd[50];
               if(!endstops.endstop_state && cur_endstops_state){
                               sprintf_P(cmd, PSTR("endstop_state: x%d  y%d z%d"), x_state,y_state,z_state);
                               ui.set_alert_status_P(cmd);
                               endstops_hit = true;
               }
               else
                 endstops.endstop_state =false;

               if(     endstops_hit && !cur_endstops_state )
               {
                               endstops_hit = false;
...
//                  if (encoderStepRate >= ENCODER_1000X_STEPS_PER_SEC)     encoderMultiplier = 1000;
                  ...
                  //#define ENCODER_RATE_MULTIPLIER_DEBUG
                    ...
                    SERIAL_ECHO_START();
                    serial_echopair_PGM("Enc Step Rate: ", encoderStepRate);
                    serial_echopair_PGM("  Multiplier: ", encoderMultiplier);
//                    serial_echopair_PGM("  ENCODER_10X_STEPS_PER_SEC: ", ENCODER_10X_STEPS_PER_SEC);
//                    serial_echopair_PGM("  ENCODER_100X_STEPS_PER_SEC: ", ENCODER_100X_STEPS_PER_SEC);
                    SERIAL_EOL();
                  #endif
                }
  ...
  void MarlinUI::reset_status(const bool no_welcome) {
    ...
    #endif
    else if (print_job_timer.isRunning() && IS_SD_PRINTING())
